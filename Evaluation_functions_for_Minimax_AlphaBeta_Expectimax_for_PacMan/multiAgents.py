...
def betterEvaluationFunction(currentGameState):
    ...
    numberOfCapsules = len(newCapsuleList)
    min_distance_to_capsule = -1
    
    for capsule in newCapsuleList:
        distance = util.manhattanDistance(newPos,capsule)
        if min_distance_to_capsule >= distance or min_distance_to_capsule ==-1:
            min_distance_to_capsule=distance     
    ...
    for scared in newScaredTimes:        
        if scared != 0:
            for ghost_state in currentGameState.getGhostPositions():
                distance = util.manhattanDistance(newPos, ghost_state)
                distances_to_ghosts += distance
                score_hunter=score_hunter+(1/float(distances_to_ghosts))*scared
            return currentGameState.getScore() + score_hunter + 1/float(min_distance_to_capsule)+1/float(min_food_distance)
        else:
            return currentGameState.getScore() + 6/float(min_food_distance) - 1/float(distances_to_ghosts)  - numberOfCapsules - numberOfFood
